...
Frame::Frame(const Frame &frame)
  ...
  if (!frame.mTcw.empty()) mTcw = frame.mTcw.clone();
}
...
bool Frame::isInFrustum(MapPoint *pMP, float viewingCosLimit) {
  ...
  vector<float>::iterator it =
      lower_bound(mvScaleFactors.begin(), mvScaleFactors.end(), ratio);
  int nPredictedLevel = it - mvScaleFactors.begin();

  if (nPredictedLevel >= mnScaleLevels) nPredictedLevel = mnScaleLevels - 1;
  ...
  for (int ix = nMinCellX; ix <= nMaxCellX; ix++) {
    for (int iy = nMinCellY; iy <= nMaxCellY; iy++) {
      ...
      for (size_t j = 0, jend = vCell.size(); j < jend; j++) {
        ...
        if (abs(kpUn.pt.x - x) > r || abs(kpUn.pt.y - y) > r) continue;
