...
Tracking::Tracking(ORBVocabulary* pVoc, FramePublisher* pFramePublisher,
                   MapPublisher* pMapPublisher, Map* pMap, double fps)
  ...
  int nRGB;
  pnh.param("Camera/RGB", nRGB, 0);
  mbRGB = static_cast<bool>(nRGB);
  if (mbRGB)
    ROS_INFO("- color order: RGB (ignored if grayscale)");
  else
    ROS_INFO("- color order: BGR (ignored if grayscale)");

  int nFeatures;
  pnh.param("ORBextractor/nFeatures", nFeatures, 1000);
  double fScaleFactor;
  pnh.param("ORBextractor/scaleFactor", fScaleFactor, 1.2);
  int nLevels;
  pnh.param("ORBextractor/nLevels", nLevels, 8);
  int fastTh;
  pnh.param("ORBextractor/fastTh", fastTh, 20);
  int nScore;
  pnh.param("ORBextractor/nScoreType", nScore, 1);
  assert(nScore == 1 || nScore == 0);

  mpORBextractor =
      new ORBextractor(nFeatures, fScaleFactor, nLevels, nScore, fastTh);
  ROS_INFO("ORB Extractor Parameters: ");
  ROS_INFO("- Number of Features: %d", nFeatures);
  ROS_INFO("- Scale Levels: %d", nLevels);
  ROS_INFO("- Scale Factor: %0.2f", fScaleFactor);
  ROS_INFO("- Fast Threshold: %d", fastTh);
  if (nScore)
    ROS_INFO("- Score: FAST");
  else
    ROS_INFO("- Score: HARRIS");

  // ORB extractor for initialization
  // Initialization uses only points from the finest scale level
  mpIniORBextractor = new ORBextractor(nFeatures * 2, 1.2, 8, nScore, fastTh);

  int nMotion;
  pnh.param("UseMotionModel", nMotion, 1);
  mbMotionModel = static_cast<bool>(nMotion);
  if (mbMotionModel) {
    mVelocity = cv::Mat::eye(4, 4, CV_32F);
    ROS_INFO("Motion Model: Enabled");
  } else {
    ROS_INFO(
...
Tracking::Tracking(ORBVocabulary* pVoc, FramePublisher* pFramePublisher,
  ...
  if (mbMotionModel) {
    ...
    cout << endl << "Motion Model: Disabled (not recommended, change settings "
                    "UseMotionModel: 1)" << endl << endl;
...
void Tracking::Run2() {
  ...
  const auto image_topic = pnh.resolveName("image");
  image_transport::CameraSubscriber sub_camera;
  sub_camera = it.subscribeCamera(image_topic, 1, &Tracking::GrabCamera, this);
  ROS_INFO("Subscribing to topic: %s", image_topic.c_str());

  ros::spin();
}

void Tracking::GrabCamera(const sensor_msgs::ImageConstPtr& image_msg,
                          const sensor_msgs::CameraInfoConstPtr& cinfo_msg) {
  if (!mbCameraInit) {
    mK = cv::Mat::eye(3, 3, CV_32F);
    auto fx = static_cast<float>(cinfo_msg->K[0]);  // fx
    auto fy = static_cast<float>(cinfo_msg->K[4]);  // fy
    auto cx = static_cast<float>(cinfo_msg->K[2]);  // cx
    auto cy = static_cast<float>(cinfo_msg->K[5]);  // cy
    mK.at<float>(0, 0) = fx;
    mK.at<float>(1, 1) = fy;
    mK.at<float>(0, 2) = cx;
    mK.at<float>(1, 2) = cy;
    ...
    auto k1 = static_cast<float>(cinfo_msg->D[0]);
    auto k2 = static_cast<float>(cinfo_msg->D[1]);
    auto p1 = static_cast<float>(cinfo_msg->D[2]);
    auto p2 = static_cast<float>(cinfo_msg->D[3]);
    mDistCoef = cv::Mat(4, 1, CV_32F);
    mDistCoef.at<float>(0) = k1;
    mDistCoef.at<float>(1) = k2;
    mDistCoef.at<float>(2) = p1;
    mDistCoef.at<float>(3) = p2;
    ROS_INFO("- k1: %f, k2: %f, p1: %f, p2: %f", k1, k2, p1, p2);
    mbCameraInit = true;
    return;
  }

  // Forward image to GrabImage
  GrabImage(image_msg);
}
...
void Tracking::Run() {
  ...
  ros::Subscriber sub =
      nodeHandler.subscribe("/camera/image_raw", 1, &Tracking::GrabImage, this);
...
void Tracking::GrabImage(const sensor_msgs::ImageConstPtr& msg) {
  ...
  if (cv_ptr->image.channels() == 3) {
    if (mbRGB)
      cv::cvtColor(cv_ptr->image, im, CV_RGB2GRAY);
    else
      cv::cvtColor(cv_ptr->image, im, CV_BGR2GRAY);
  ...
  if (mState == WORKING || mState == LOST)
    ...
    mCurrentFrame = Frame(im, cv_ptr->header.stamp.toSec(), mpIniORBextractor,
                          mpORBVocabulary, mK, mDistCoef);
...
void Tracking::Initialize() {
  // Check if current frame has enough keypoints, otherwise reset initialization
  // process
  if (mCurrentFrame.mvKeys.size() <= 100) {
...
bool Tracking::TrackPreviousFrame() {
  ...
  if (nmatches < 10) {
    nmatches = matcher.WindowSearch(mLastFrame, mCurrentFrame, 100,
                                    vpMapPointMatches, 0);
    if (nmatches < 10) {
      vpMapPointMatches = vector<MapPoint*>(mCurrentFrame.mvpMapPoints.size(),
  ...
  for (size_t i = 0, iend = mCurrentFrame.mvpMapPoints.size(); i < iend; i++) {
    if (mCurrentFrame.mvpMapPoints[i]) {
      ...
      if (!pMP->isBad()) {
        ...
        for (map<KeyFrame*, size_t>::iterator it = observations.begin(),
...
bool Tracking::Relocalisation() {
  ...
  // Relocalisation is performed when tracking is lost and forced at some stages
  // during loop closing
  ...
  while (nCandidates > 0 && !bMatch) {
    for (size_t i = 0; i < vpCandidateKFs.size(); i++) {
      ...
      if (!Tcw.empty()) {
        ...
        for (size_t io = 0, ioend = mCurrentFrame.mvbOutlier.size(); io < ioend;
             io++)
        ...
        if (nGood < 50) {
          ...
          if (nadditional + nGood >= 50) {
            ...
            if (nGood > 30 && nGood < 50) {
              sFound.clear();
              for (size_t ip = 0, ipend = mCurrentFrame.mvpMapPoints.size();
                   ip < ipend; ip++)
