...
bool LoopClosing::DetectLoop() {
  ...
  // If the map contains less than 10 KF or less than 10KF have passed from last
  // loop detection
...
bool LoopClosing::ComputeSim3() {
  ...
  for (int i = 0; i < nInitialCandidates; i++) {
    ...
    if (nmatches < 20) {
      ...
      Sim3Solver* pSolver =
          new Sim3Solver(mpCurrentKF, pKF, vvpMapPointMatches[i]);
    ...
    for (int i = 0; i < nInitialCandidates; i++) {
      ...
      if (!Scm.empty()) {
        ...
        const int nInliers = Optimizer::OptimizeSim3(
            mpCurrentKF, pKF, vpMapPointMatches, gScm, 10);
