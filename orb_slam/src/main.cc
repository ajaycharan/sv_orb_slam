...
int main(int argc, char** argv) {
  ...
  cout << endl << "ORB-SLAM Copyright (C) 2014 Raul Mur-Artal" << endl
       ...
       << endl << "under certain conditions. See LICENSE.txt." << endl;

  if (argc != 3) {
    cerr << endl << "Usage: rosrun ORB_SLAM ORB_SLAM path_to_vocabulary "
                    "path_to_settings (absolute or relative to package "
                    "directory)" << endl;
    ros::shutdown();
    return 1;
  }
  ...
  if (!fsSettings.isOpened()) {
    ROS_ERROR(
        "Wrong path to settings. Path must be absolut or relative to ORB_SLAM "
        "package directory.");
    ros::shutdown();
    return 1;
  }
  ...
  if (!fsVoc.isOpened()) {
    cerr << endl << "Wrong path to vocabulary. Path must be absolut or "
                    "relative to ORB_SLAM package directory." << endl;
    ros::shutdown();
    return 1;
  }
  ...
  cout << endl << "Saving Keyframe Trajectory to KeyFrameTrajectory.txt"
       << endl;
  string strFile =
