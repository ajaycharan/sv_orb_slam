  ...
  while (ros::ok()) {
    ...
    if (CheckNewKeyFrames()) {
      ...
      if (!CheckNewKeyFrames() && !stopRequested()) {
        ...
        if (!CheckNewKeyFrames()) SetAcceptKeyFrames(true);
      }
...
void LocalMapping::ProcessNewKeyFrame() {
  ...
  if (mpCurrentKeyFrame->mnId == 0) return;

  // Associate MapPoints to the new keyframe and update normal and descriptor
  vector<MapPoint*> vpMapPointMatches = mpCurrentKeyFrame->GetMapPointMatches();
  if (mpCurrentKeyFrame->mnId > 1)  // This operations are already done in the
                                    // tracking for the first two keyframes
  {
...
void LocalMapping::CreateNewMapPoints() {
  ...
  vector<KeyFrame*> vpNeighKFs =
      mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(20);
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    // Small translation errors for short baseline keyframes make scale to
    // diverge
    ...
    for (size_t ikp = 0, iendkp = vMatchedKeysUn1.size(); ikp < iendkp; ikp++) {
      ...
      if (cosParallaxRays < 0 || cosParallaxRays > 0.9998) continue;
      ...
      float ratioOctave = mpCurrentKeyFrame->GetScaleFactor(kp1.octave) /
                          pKF2->GetScaleFactor(kp2.octave);
...
void LocalMapping::SearchInNeighbors() {
  ...
  for (vector<KeyFrame*>::iterator vit = vpNeighKFs.begin(),
                                   vend = vpNeighKFs.end();
       vit != vend; vit++) {
    KeyFrame* pKFi = *vit;
    ...
    for (vector<KeyFrame*>::iterator vit2 = vpSecondNeighKFs.begin(),
