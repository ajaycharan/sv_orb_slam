...
int Optimizer::PoseOptimization(Frame* pFrame) {
  ...
  // From second to final optimization we include only inliers in the
  // optimization
...
void Optimizer::OptimizeEssentialGraph(
  ...
  Eigen::Matrix<double, 7, 7> matLambda =
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    for (vector<KeyFrame*>::iterator vit = vpConnectedKFs.begin();
